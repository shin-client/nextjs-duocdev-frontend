...
import { usePayForGuest } from "@/queries/useOrder";
import {
  GetOrdersResType,
...
export default function OrderGuestDetail({
  ...
  const { mutateAsync: payForGuest, isPending } = usePayForGuest();

  const pay = async () => {
    try {
      const result = await payForGuest({ guestId: guest?.id as number });
